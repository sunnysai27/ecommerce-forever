...
const Orders = ({token}) => {
  ...
  const statusHandler = async (e, order_id) => {
    ...
    try {
      ...
      const response = await axios.post( backendUrl + '/api/orders/status' , {order_id,status: e.target.value}, {headers: {token}});
        ...
        <div>
          {
            orders.map((order , index) => (
              <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-3 items-start border-2 border-gray-200 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-gray-700' key={index}>
                <img className='w-12' src={assets.parcel_icon} alt='alt' />
                <div>
                  <div>
                  { order.items.map((item , ind) => {
                    if(ind === order.items.length -1 ) {
                      return <p className='py-0.5' key={ind} > {item.name} X {item.quantity} <span>{item.size}</span> </p>
                    } else {
                      return <p className='py-0.5' key={ind} > {item.name} X {item.quantity} <span>{item.size}</span> </p>
                    }
